...
//         <nav className="hidden lg:flex items-center justify-start gap-8 pl-18">
...
//                 isScrolled ? "h-26 md:h-28" : "h-26 md:h-30",
...
//               to="/contact"
//               className={cn(
//                 "bg-[#f5f5f0] flex items-center justify-center px-[33px] py-[9px] relative rounded-full transition-colors border-none hover:bg-[#eaeaea]",
//                 "text-[#1a3c34] hover:text-[#1a3c34]"
//               )}
//             >
    ...
    >
      <div className="relative container mx-auto px-6 grid grid-cols-[1fr_auto_1fr] items-center gap-4">
        ...
        <div className="lg:hidden flex justify-start">
          <button
            className={cn(
              "z-[110] p-2 -ml-2 relative group transition-colors w-10 h-10 flex items-center justify-center",
              useWhiteText && !isScrolled && !isMobileMenuOpen ? "text-white" : "text-primary"
            )}
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            aria-label="Toggle Menu"
          >
        ...
        {/* Logo - Centered */}
        <div className="flex justify-center items-center -ml-3">
          <Link to="/" className="z-[110] relative block group">
            <img
              src={Logo}
              alt="Haritha Farms"
              className={cn(
                "transition-all duration-700 ease-[0.16,1,0.3,1] w-auto object-contain",
                isScrolled || isMobileMenuOpen ? "h-20 md:h-24" : "h-26 md:h-30",
                useWhiteText && !isScrolled && !isMobileMenuOpen && "brightness-0 invert"
              )}
            />
          </Link>
        </div>
        ...
        <div className="hidden lg:flex items-center justify-end gap-8">
          ...
          >
            <Link
              to="/contact"
              className="bg-[#f5f5f0] flex items-center justify-center px-[33px] py-[9px] relative rounded-full transition-colors border-none hover:bg-[#eaeaea] text-[#1a3c34]"
            >
              <div aria-hidden="true" className="absolute border border-[rgba(26,60,52,0.2)] inset-0 pointer-events-none rounded-full" />
              <span className="font-bold relative shrink-0 text-[#1a3c34] text-[10px] tracking-[1.5px] uppercase">
                ORDER ENQUIRIES
              </span>
            </Link>
          </Button>
        ...
        {/* Mobile Right Side - Order Link */}
        <div className="lg:hidden flex justify-end">
          <Link
            to="/contact"
            className={cn(
              "z-[110] text-[10px] font-bold uppercase tracking-widest border border-current rounded-full px-4 py-2 transition-colors",
              useWhiteText && !isScrolled && !isMobileMenuOpen ? "text-white border-white/30" : "text-primary border-primary/20"
            )}
          >
            VISIT
          </Link>
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.4, ease: "easeInOut" }}
              className="fixed inset-0 top-0 bg-background z-[105] flex flex-col pt-32 px-6 lg:hidden overflow-y-auto min-h-screen pointer-events-auto"
            >
              <nav className="flex flex-col items-center gap-6 text-center w-full max-w-md mx-auto pb-12">
                {navLinks.map((link, i) => (
                  <motion.div
                  ...
                  >
                    <Link
                      to={link.path}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="text-2xl font-serif text-primary hover:text-secondary transition-colors block w-full py-2"
                    >
                      {link.name}
                    </Link>
                    {link.dropdown && (
                      <div className="flex flex-col gap-3 pt-1">
                        {link.dropdown.map((dropItem) => (
                          <Link
                            ...
                            to={dropItem.path}
                            onClick={() => setIsMobileMenuOpen(false)}
                            className="text-sm font-sans tracking-widest uppercase text-muted-foreground hover:text-primary transition-colors block w-full py-1"
                          >
                            {dropItem.name}
                          </Link>
                        ))}
                ...
                >
                  <Button
                    asChild
                    className="w-full bg-primary text-white hover:bg-secondary rounded-full uppercase tracking-widest h-14 text-sm"
                  >
                    <Link to="/contact" onClick={() => setIsMobileMenuOpen(false)}>Contact Us</Link>
                  </Button>
                </motion.div>
